...
| **server-action** | "server action", "form action", "CRUD", "create/update/delete", "file upload action", "'use server'" | Usuario quiere crear logica en `src/actions/`; feature tiene types/services pero no actions | Feature con UI lista pero sin logica server-side; formulario que necesita backend | 2+ pts |
...
### D: Aplicacion Completa (Full Path)

```
Trigger: /new-app seguido de implementacion

Secuencia:
  1. /new-app (command)      → Generar BUSINESS_LOGIC.md
  2. /add-login (command)    → Sistema de autenticacion
  3. /harden                 → Seguridad core
  4. /feature-scaffold (xN)  → Una por feature del BUSINESS_LOGIC
  5. /supabase-patterns      → DB setup por feature
  6. /server-action (xN)     → Actions por feature
  7. /docker-deploy          → Cuando listo para produccion
```
...
2. SIGNAL DETECTION
   - Evaluar las 7 filas de la matriz
...
SKILL CATALOG:
  ...
  /ai-engine         → Vercel AI SDK, provider routing, BYOK, cross-model review
